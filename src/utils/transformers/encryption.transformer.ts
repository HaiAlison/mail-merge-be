...
export const encryptionTransformer: ValueTransformer = {
  to(value: string | null): string | null {
    ...
    if (!secretKey || secretKey.length !== 32) {
      console.warn(
        'ENCRYPTION_KEY is missing or invalid (must be 32 chars). Tokens will not be encrypted properly.',
      );
      ...
      throw new Error(
        'ENCRYPTION_KEY is required and must be 32 characters long for AES-256-GCM.',
      );
    }
  ...
  from(value: string | null): string | null {
    ...
    if (!secretKey || secretKey.length !== 32) {
      throw new Error(
        'ENCRYPTION_KEY is required and must be 32 characters long to decrypt tokens.',
      );
    }
